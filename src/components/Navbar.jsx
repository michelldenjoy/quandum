        ...
        >
          ...
          <div className={`absolute inset-0 transition-transform duration-500 ease-out ${submenuOpen ? "-translate-x-full" : "translate-x-0"}`}>
            <div className="h-full overflow-y-auto py-32 pb-24 px-6 sm:px-8 space-y-6">
              {links.map((link) => (
                <div key={link.name}>
                  {link.dropdown ? (
                    <button onClick={() => setSubmenuOpen(link.name)} className="w-full flex items-center justify-between text-xl tracking-widest uppercase font-bold text-white border-b border-neutral-800 pb-6">
                      <span>{link.name}</span>
                      <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                    </button>
                  ) : (
                    <Link to={link.path} onClick={() => setOpen(false)} className="block py-6 text-xl font-bold text-white border-b border-neutral-800 uppercase tracking-widest">{link.name}</Link>
                  )}
                </div>
              ))}
            </div>
          ...
          <div className={`absolute inset-0 transition-transform duration-500 ease-out ${submenuOpen ? "translate-x-0" : "translate-x-full"}`}>
            <div className="h-full overflow-y-auto py-32 pb-24 px-6 sm:px-8">
              ...
              <h2 className="text-2xl font-bold text-white mb-8 border-b border-neutral-800 pb-4">{submenuOpen}</h2>
              <div className="space-y-4">
                {links.find(l => l.name === submenuOpen)?.dropdown?.map(item => (
                  <Link key={item.name} to={item.path} onClick={() => setOpen(false)} className="block py-4 text-lg text-gray-300">{item.name}</Link>
                ))}
              </div>
            </div>
          </div>
